...
class Application_Model extends CI_Model {
  ...
  private function __get_date_update_data() {
    return array(
  ...
  protected function set_before_update($update_data = array()) {
    $merge_data = array();
    foreach ($update_data as $key => $value) {
      if (is_array($value)) {
        $merge_data[] = array_merge($value, $this->__get_date_update_data());
      } else {
        $merge_data = array_merge($update_data, $this->__get_date_update_data());
      }
    }
    return $merge_data;
  }
